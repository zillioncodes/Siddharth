...
import edu.buffalo.cse.ir.wikiindexer.tokenizer.rules.DelimitorRule;
...
public class TokenizerFactory {
	...
	public Tokenizer getTokenizer(INDEXFIELD field) {
		...
		case AUTHOR:
			try {
				tokenizer = new Tokenizer(new AccentRule(), new DelimitorRule());
		...
		case CATEGORY:
			try {
				tokenizer = new Tokenizer(new AccentRule(), new SpecialCharRule());
			} catch (TokenizerException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		case TERM:
			try {
				tokenizer = new Tokenizer(new AccentRule(), new CapitalizationRule(),
						new SpecialCharRule(), new PunctuationRule(),
						new HyphenRule(), new ApostropheRule(), new DateRule(),
						new NumberRule(), new WhitespaceRule(),
						new StopWordsRule(), new EnglishStemmer());
			} catch (TokenizerException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
